...
export async function generateEvaluationPDF(ev, config) {
  ...
  const aRowH = 15
  // header (day numbers)
  cell(p2, M, aTop, mLabelW, aRowH, 'MES', { bold: true, size: 7, bg: DARK_BG })
  for (let d = 1; d <= 31; d++) {
  ...
  for (let m = 0; m < 12; m++) {
    ...
    const dim = daysInMonth(ev.anio || config?.anio || new Date().getFullYear(), m)
    const monthData = ev.attendance?.[m] || {}
    for (let d = 1; d <= 31; d++) {
      if (d > dim) {
        cell(p2, M + mLabelW + (d - 1) * dayW, ay, dayW, aRowH, '', { bg: rgb(0.78, 0.78, 0.78) })
      } else {
        cell(p2, M + mLabelW + (d - 1) * dayW, ay, dayW, aRowH, monthData[d] || '', { size: 5.5, bold: true })
      }
    }
    ay -= aRowH
  }
